...
class EditableBoxed {
    ...
    _createInstance() {
      const _this = this;
      this._datatable = jQuery('#datatableBoxed_penjualan_cash').DataTable({
    ...
    _addListeners() {
      ...
      document.querySelectorAll('.add-datatable-penjualanCash').forEach((el) => el.addEventListener('click', this._onAddRowClick.bind(this)));
      ...
      document.getElementById('addEditModal_penjualan_cash').addEventListener('hidden.bs.modal', this._clearModalForm);
    }
    ...
    _initBootstrapModal() {
      this._addEditModal = new bootstrap.Modal(document.getElementById('addEditModal_penjualan_cash'));
    }
    ...
    _onAddRowClick() {
      this._showModal('tambah', 'Tambah Data Penjualan Cash', 'Tambah');
    }
