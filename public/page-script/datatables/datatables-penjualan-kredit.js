...
class EditableBoxed {
    ...
    _createInstance() {
      const _this = this;
      this._datatable = jQuery('#datatableBoxed_penjualan_kredit').DataTable({
    ...
    _addListeners() {
      ...
      document.querySelectorAll('.add-datatable-penjualanKredit').forEach((el) => el.addEventListener('click', this._onAddRowClick.bind(this)));
      ...
      document.getElementById('addEditModal_penjualan_kredit').addEventListener('hidden.bs.modal', this._clearModalForm);
    }
    ...
    _initBootstrapModal() {
      this._addEditModal = new bootstrap.Modal(document.getElementById('addEditModal_penjualan_kredit'));
    }
    ...
    _onAddRowClick() {
      this._showModal('tambah', 'Tambah Data Penjualan Kredit', 'Tambah');
    }
